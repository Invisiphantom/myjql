...
void hash_table_insert(BufferPool* pool, short size, off_t addr) {
    HashMapDirectoryBlock* dir_block = (HashMapDirectoryBlock*)get_page(
        pool, (size / HASH_MAP_DIR_BLOCK_SIZE + 1) * PAGE_SIZE);  //* 锁定目录块
    off_t curAddr = dir_block->directory[size % HASH_MAP_DIR_BLOCK_SIZE];

    // 如果目录项为空，则新建一个哈希块
    if (curAddr == 0) {
        curAddr = hash_table_alloc(pool);
        dir_block->directory[size % HASH_MAP_DIR_BLOCK_SIZE] = curAddr;     // 连接新块
        HashMapBlock* hash_block = (HashMapBlock*)get_page(pool, curAddr);  //* 锁定新块
        hash_block->table[0] = addr;
        hash_block->n_items = 1;
        release(pool, curAddr);                                           //* 释放新块
    ...
    release(pool, (size / HASH_MAP_DIR_BLOCK_SIZE + 1) * PAGE_SIZE);  //* 释放
    return;
}
...
off_t hash_table_pop_lower_bound(BufferPool* pool, short size) {
    ...
    for (int i = size; i < ctrl_max_size; i++) {
        ...
        if (block_addr != 0) {
            ...
            block->n_items--;
            if (block->n_items == 0) {
                dir_block->directory[i % HASH_MAP_DIR_BLOCK_SIZE] = block->next;  // 连接新块
                release(pool, block_addr);          //* 释放哈希块
                hash_table_free(pool, block_addr);  // 释放旧块至空闲链表
            } else {
                release(pool, block_addr);  //* 释放哈希块
            }
...
void hash_table_pop(BufferPool* pool, short size, off_t addr) {
    ...
    if (curAddr == 0) {
        release(pool, (size / HASH_MAP_DIR_BLOCK_SIZE + 1) * PAGE_SIZE);  //* 释放目录块
        fprintf(stderr, "Deltete: dir_addr not found\n");
        assert(0);
    }

    off_t befAddr = 0;
    while (curAddr != 0) {
        HashMapBlock* block = (HashMapBlock*)get_page(pool, curAddr);  //* 锁定哈希块
        for (int i = 0; i < block->n_items; i++) {
            if (block->table[i] == addr) {
                ...
                // 如果块为空, 则删除块
                if (block->n_items == 0) {
                    if (befAddr == 0) {  // 如果是第一个块, 则连接到目录块
                        dir_block->directory[size % HASH_MAP_DIR_BLOCK_SIZE] =
                            block->next;                 // 连接新块
                        release(pool, curAddr);          //* 释放哈希块
                        hash_table_free(pool, curAddr);  // 释放旧块至空闲链表
                    } else {
                ...
                release(pool,
                        (size / HASH_MAP_DIR_BLOCK_SIZE + 1) * PAGE_SIZE);  //* 释放目录块
                return;
            }
        }
    ...
    fprintf(stderr, "Deltete: Addr not found\n");
    assert(0);
    return;
}
